...
select sum(m.me_price)
from menu_item m join cart c on m.me_id=c.ct_pr_id join user123 u on u.us_id=c.ct_us_id where u.us_id IN(select 
us_id from user123 where us_name='surya')


select u.us_name,m.me_name,m.me_price,m.me_active,m.me_category,m.me_free_delivery 
from menu_item m join cart c join user123 u on m.me_id=c.ct_pr_id and u.us_id=c.ct_us_id
